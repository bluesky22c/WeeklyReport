...
| 15년 이상 | 4.5 | 고난도 기술/현장 이슈 주도, 복합 프로젝트 책임, 후배 지원 |
| 10년 이상 | 4.3 | 주요 프로젝트 독립 수행, 문제 해결, 일정·품질 책임 |
| 8년 이상 | 4.0 | 안정적 실무 수행, 반복 이슈 개선, 협업 기여 |
| 5년 이상 | 3.6 | 담당 기능 완결, 이슈 대응, 실무 숙련도 향상 |
...
금주실적 본문에 `RAS Vision Part` 단서가 있으면 대표 프로젝트명 `RAS Vision 알고리즘 개발`로 분류한다.
...
비전 알고리즘 개발, 검사 알고리즘 개발, 픽셀 검사 알고리즘, 이미지 분석/매핑 알고리즘 등 핵심 Vision 알고리즘 개발성 업무는 기술적 난이도 최상위권으로 보고 4.8을 적용한다.

| 세부 프로젝트 | 난이도 | 판단 기준 |
|----------------|-------:|-----------|
| Vision 알고리즘 개발 | 4.8 | 비전 검사 알고리즘, 픽셀 검사, 이미지 분석, 매핑/Addressing, 불량 판정 로직 등 핵심 알고리즘 개발 |
| 검사 알고리즘 개발 | 4.8 | 검사 판정 조건, 불량 코드, 이미지 처리, 알고리즘 비교·검증 업무 |
...
| RAS Vision Part | 4.8 | `RAS Vision 알고리즘 개발`로 통합 인식해 계산 |
| RAS Vision 알고리즘 개발 | 4.8 | RAS 업무 중 검사 알고리즘, 이미지 분석, 픽셀 매핑, Addressing, 불량 판정 로직 개발 |
| TOE CIM | 3.5 | TOE 운영 프로그램과 분리해 계산 |
| TOE 운영 프로그램 | 4.7 | TOE 운영 프로그램, 셋업, 원버튼, TSP 검사, 안전 검수 |
| WHTM ACAControlSystem | 4.3 | G6 ACAControlSystem, Recipe, Lot Loading, Alarm, AGV/EIP 통신 |
...
| Vision UI/WPF Framework | 4.6 | WPF 기반 Vision UI, 이미지 뷰어/렌더링, ROI/Overlay, Framework, DataGrid/PropertyGrid 구성 |
...
- `RAS Vision Part`는 `RAS Vision 알고리즘 개발`로 인식해 계산한다.
- `LGD LifeTimeSystem`, `LGD ICBSystem`은 `LGD ICBSystem`으로 인식해 계산한다.

| 프로젝트 | 확정 투입인원 | 투입자 |
|----------|-------------:|--------|
| TOE 운영 프로그램 | 2명 | 김현석, 김성현 |
| TOE 1차 설비 | 1명 | 배유민 |
| TOE CIM | 1명 | 김성현 |
| 교육/학습 | 1명 |  |
| ETC | 1명 |  |
...
- `RAS Vision Part`는 `RAS Vision 알고리즘 개발`로 통합해 평가한다.
...
- 김도현 RAS Vision Part 항목은 `RAS Vision 알고리즘 개발`로 분류한다.
- 박주상 RAS 업무 중 알고리즘/이미지 분석/매핑/불량 판정은 `RAS Vision 알고리즘 개발`로 분류한다.
...
> ⚠️ srv.py는 Codex의 Windows PowerShell 실행 환경에서 시작해야 `D:\` 경로에 접근 가능함.
> Linux/bash 환경에서 시작하면 `D:\` 경로에 접근 불가.
...
> ⚠️ **반드시 Windows PowerShell 환경에서 실행할 것.**
> Linux bash 샌드박스는 CIFS 마운트 캐시 문제로 최신 파일을 읽지 못할 수 있음.

Codex의 PowerShell 명령 실행으로 실행:
```
...
파일 유효성 검증 스크립트 (Windows PowerShell에서 실행):
```
...
매주 보고서가 추가되면 Codex에게 다음과 같이 요청:

> **"추가된 주간업무보고 저장해줘"**

Codex는 이 AGENTS.md를 읽고 위 절차를 자동 수행:
1. 기존 저장 파일 목록 확인
2. 그룹웨어에서 신규 항목 파악
3. 내용 수집 → HTML 저장
